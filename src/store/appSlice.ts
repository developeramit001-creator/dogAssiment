...
const slice = createSlice({
  ...
  reducers: {
    ...
    toggleFavorite: (s, a: PayloadAction<string>) => {
      s.favorites = s.favorites.includes(a.payload)
        ? s.favorites.filter(x => x !== a.payload)
        : [...s.favorites, a.payload];
    },
    setFilters: (s, a: PayloadAction<FilterState>) => {
      s.filters = a.payload;
    },
    setOnboarded: (s, a: PayloadAction<boolean>) => {
      s.onboarded = a.payload;
    },
    resetFilters: s => {
      s.filters = { ...EMPTY_FILTERS, groups: [], sizes: [], coats: [] };
    },
  },
});
export const {
...
export const hydrateApp = () => async (dispatch: any) => {
  try {
    ...
    if (f) dispatch(setFavorites(JSON.parse(f)));
    if (o) dispatch(setOnboarded(o === '1'));
  } catch {}
};
export const persistFavorites = (favorites: string[]) => async () => {
