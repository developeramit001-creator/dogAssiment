...
export default function FavoritesScreen() {
  ...
  const list = breeds.filter(b => favorites.includes(b.id));
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      <Text style={styles.title}>Saved breeds</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <BreedCard
            ...
            favorite
            onFavorite={() => {
              const next = favorites.filter(x => x !== item.id);
              dispatch(toggleFavorite(item.id));
              dispatch(persistFavorites(next) as never);
            }}
            onPress={() => nav.navigate('BreedDetail', { id: item.id })}
          />
        )}
        ListEmptyComponent={
          <View style={styles.empty}>
            <Icon name="heartOutline" size={52} color={colors.peach} />
