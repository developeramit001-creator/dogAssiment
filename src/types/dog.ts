...
export type BreedAttributes = {
  ...
  male_height?: Range;
  female_height?: Range;
  origin?: { era?: string; region?: string; country?: string };
  coat?: { type?: string; colors?: string[]; length?: string };
  traits?: Traits;
  other_names?: string[];
...
export type Breed = {
  ...
  relationships?: { group?: { data?: { id: string; type: string } } };
};
export type Group = { id: string; type: 'group'; attributes: { name: string } };
export type ApiPage = {
  data: Breed[];
  meta: { pagination: { current: number; last: number; records: number } };
};
